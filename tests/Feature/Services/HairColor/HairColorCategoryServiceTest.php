...
/**
 * HairColorCategoryService 业务规则验证：
 ...
 */
class HairColorCategoryServiceTest extends TestCase
{
    use RefreshDatabase;

    private HairColorCategoryService $service;

    protected function setUp(): void
    {
        parent::setUp();

        $this->service = new HairColorCategoryService();
    }
    ...
    {
        $category = $this->service->create(['name' => '自然色系', 'slug' => 'natural-'.uniqid()]);
    ...
    /**
     * 局部更新：只传 name，不传 slug / parent_id / description 等其它字段，
     * 不应因为访问不存在的数组键而报错，且未修改字段应保持模型当前值不变。
     */
    public function test_update_allows_partial_fields_without_touching_others(): void
    {
        $parent = $this->makeCategory();
        $category = $this->makeCategory(['parent_id' => $parent->id, 'sort' => 5]);

        $updated = $this->service->update($category, ['name' => '新名称']);
    ...
    public function test_delete_rejects_when_undeleted_hair_colors_exist(): void
    {
        $category = $this->makeCategory();
    ...
    public function test_restore_brings_back_soft_deleted_category(): void
    {
        ...
        $this->assertDatabaseHas('hair_color_categories', ['id' => $restored->id, 'deleted_at' => null]);
    }
    ...
    public function test_restore_does_not_automatically_restore_children(): void
    {
        ...
        $this->assertDatabaseHas('hair_color_categories', ['id' => $parent->id, 'deleted_at' => null]);
        $this->assertSoftDeleted('hair_color_categories', ['id' => $child->id]);
    }
