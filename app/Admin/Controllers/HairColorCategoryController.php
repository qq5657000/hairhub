...
use App\Services\HairColor\HairColorCategoryService;
...
/**
 * 发色分类 Dcat Admin 后台管理。
 *
 * 严格复用第一阶段已完成的 HairColorCategoryService，本控制器不重复实现分类层级
 * 约束、slug 唯一性、删除前引用检查等业务逻辑：
 * - 新增/编辑统一通过 handleSaving() 调用 HairColorCategoryService::create()/update()，
 *   保存前的封面校验（图片类型/启用状态）交给 ManagesCoverMedia 通用能力，最终写入的
 *   cover_media_id 随其它字段一起传给 Service，由 Service 完成真正的持久化；
 * - 软删除统一通过 handleDeleting() 调用 HairColorCategoryService::delete()；
 * - 恢复统一通过 restore() 调用 HairColorCategoryService::restore()。
 *
 * V1.0 只保留软删除和恢复，不提供永久删除（物理删除）能力：分类和发色是长期
 * 数据资产，且当前无法确认所有未来业务引用，不因为存在回收站就默认开放物理
 * 删除入口（对应本次 Review 结论）。
 *
 * saving()/deleting() 回调始终返回非空 JsonResponse，短路 Dcat 默认的
 * store()/update()/destroy() 持久化流程（做法与 HairstyleController 一致），
 * 确保“保存/删除”只有 Service 这一个真正的写入入口。
 */
class HairColorCategoryController extends AdminController
{
    use ManagesCoverMedia;

    private HairColorCategoryService $categoryService;

    private MediaFileService $mediaFileService;

    public function __construct(HairColorCategoryService $categoryService, MediaFileService $mediaFileService)
    {
    ...
    {
        return Grid::make(new HairColorCategory(), function (Grid $grid) {
            // with() 预加载 parent/coverMedia，withCount() 统计发色数量，
            // 避免在下面的列 display() 中逐行触发关联查询（N+1）。
            $grid->model()
                ->with(['parent', 'coverMedia'])
                ->withCount('hairColors')
                ->orderByDesc('sort')
                ->orderByDesc('id');
            ...
            $grid->column('cover', '分类封面')->display(function () {
                return HairColorCategoryController::renderCoverThumb($this->coverMedia);
            });
            $grid->column('status', '状态')->using(CommonStatus::options());
            $grid->column('sort', '排序')->sortable();
            $grid->column('hair_colors_count', '发色数量')->sortable();
            ...
            $grid->filter(function (Grid\Filter $filter) {
                ...
                $filter->equal('parent_id', '父级分类')->select(HairColorCategoryController::parentFilterOptions());
            ...
            $grid->actions(function (Grid\Displayers\Actions $actions) {
                ...
                if (! $model instanceof HairColorCategory) {
                    return;
                }
                ...
                if ($model->trashed()) {
                    ...
                    $actions->append(HairColorCategoryController::renderRestoreAction($model));
                }
            });
        });
    }
    ...
    protected function detail($id)
    {
        ...
        $category = HairColorCategory::withTrashed()->with(['parent', 'coverMedia'])->findOrFail($id);

        return Show::make($id, new HairColorCategory(), function (Show $show) use ($category) {
            $show->field('id');
            ...
            $show->field('cover_display', '封面')->as(function () use ($category) {
                return HairColorCategoryController::renderCoverPreview($category->coverMedia);
            })->unescape();
    ...
    protected function form()
    {
        ...
        return Form::make(new HairColorCategory(), function (Form $form) use ($self) {
            $id = $form->getKey();

            $form->display('id');

            $form->select('parent_id', '父级分类')
                ->options(HairColorCategoryController::parentFormOptions($id))
                ->default(0)
                ->help('只允许选择状态为“启用”的顶级分类；顶级分类请选择“顶级分类”，V1.0 最多支持两级分类。如果当前父级已被禁用，下拉会保留并展示当前值，但不能切换到其它已禁用的顶级分类');
            ...
            $category = $id ? HairColorCategory::withTrashed()->with('coverMedia')->find($id) : null;

            $form->html(HairColorCategoryController::renderCoverPreview($category?->coverMedia), '当前封面');

            $form->image('cover_upload', '上传新封面')
                ->url('hair-color-categories/cover-upload')
                ->autoSave(false)
    ...
    /**
     * 恢复一个已软删除的分类：调用 HairColorCategoryService::restore()，
    ...
    public function restore($id): RedirectResponse
    {
        ...
        return redirect(admin_url('hair-color-categories'));
    }
     ...
     * 调用 HairColorCategoryService::create()/update() 完成真正的持久化，
     * 避免出现“分类已保存但封面处理失败”的半成功状态。
     *
    ...
    private function handleSaving(Form $form): JsonResponse
    {
        ...
        try {
            $category = DB::transaction(function () use ($id, $attributes, $uploadMediaId, $selectMediaId, $clearCover) {
                if ($id) {
                    /** @var HairColorCategory $category */
                    $category = HairColorCategory::withTrashed()->lockForUpdate()->findOrFail($id);
        ...
        return $id
            ? $response->refresh()
            : $response->redirect(admin_url('hair-color-categories/'.$category->id.'/edit'));
    }

    /**
     * 软删除前的统一处理：调用 HairColorCategoryService::delete()，删除前的
     * 子分类/发色引用检查完全在 Service 内完成，不在本控制器重新实现。
     *
    ...
    {
        $options = HairColorCategory::query()
            ->where('parent_id', 0)
            ->orderByDesc('sort')
            ->orderByDesc('id')
            ->pluck('name', 'id')
     ...
     * - 只允许选择状态为“启用”的顶级分类（未删除已经由 HairColorCategory 默认查询
     *   排除软删除记录保证）；
     * - 编辑时排除自身，避免把自己设置为自己的父级形成循环；
     * - 编辑时如果当前分类原本的父级分类已被禁用，额外把这一个父级加回来
     *   （标注“已禁用”），避免下拉里找不到当前值导致浏览器把 <select> 提交为
     *   其它选项、静默把 parent_id 改成不相关的值；但不会加入除“当前父级”外的
     *   其它任何禁用的顶级分类，用户不能借着下拉切换到别的禁用父级。
     *
     * @param  int|null  $id
     * @return array<int, string>
     */
    public static function parentFormOptions(?int $id): array
    {
        ...
        if ($id) {
            $currentParentId = (int) (HairColorCategory::withTrashed()->find($id)?->parent_id ?? 0);
        ...
        $query = HairColorCategory::query()
            ->where('parent_id', 0)
            ->where(function ($query) use ($currentParentId) {
    ...
    /**
     * 列表“分类封面”列渲染，只读取已预加载的 coverMedia 关联，不在渲染过程中触发新查询。
     */
    ...
    public static function renderRestoreAction(HairColorCategory $model): string
    {
        $url = admin_url('hair-color-categories/'.$model->id.'/restore');
        $token = csrf_token();
